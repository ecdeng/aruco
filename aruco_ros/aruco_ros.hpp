...
public:
    ...
    float m_ArucoDelayCompensate;

    //
    Eigen::Vector3f m_P_aruco, m_P_opti, m_P_arFromOpti;

    geometry_msgs::PoseStamped m_Pose_aruco, m_Pose_opti, m_Pose_OptiRef,m_Pose_err;
    ...
    ros::Publisher PoseErr_pub,ErrEuler_pub,optiEuler_pub ;
    rosbag::Bag m_bag;
    const string m_InstallCalibResultPath = "Aruco_InstallCalibResult.bag";
    const string m_InstallCalibInputPath = "m_InstallCalibInput.bag";
  ...
  void Calculate_Pr2w();
  bool Calculate_P_b2c(bool IsLoadFromFile);
  ...
  bool BagRead_InstallCalib();
  void InitInstallCalib();


  void UpdateInstallCalibBagFile();
  ...
  void chatter_calibInstall(const std_msgs::BoolConstPtr& msg)
  {
    cout << "received calib command" << endl;
    bool IsLoadFromFile = msg->data;
    this->Calculate_P_b2c(IsLoadFromFile);
  }
